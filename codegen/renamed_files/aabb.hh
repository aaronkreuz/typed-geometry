...
[[nodiscard]] constexpr aabb<D, ScalarT> aabb_of_pos(pos<D, ScalarT> const& v)
{
    return {v, v};
}

template <int D, class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<D, ScalarT> aabb_of_aabb(aabb<D, ScalarT, TraitsT> const& b)
{
    return aabb<D, ScalarT>(b);
}

template <int D, class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<D, ScalarT> aabb_of_sphere(sphere<D, ScalarT, D, TraitsT> const& s)
{
    return {s.center - s.radius, s.center + s.radius};
}
template <class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<2, ScalarT> aabb_of_sphere1(sphere<1, ScalarT, 2, TraitsT> const& s)
{
    const auto v = perpendicular(s.normal) * s.radius;
    return aabb_of(s.center - v, s.center + v);
}
template <class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<3, ScalarT> aabb_of_sphere2(sphere<2, ScalarT, 3, TraitsT> const& s)
{
...
template <class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<1, ScalarT> aabb_of_ellipse1(ellipse<1, ScalarT, 1, TraitsT> const& e)
{
    const auto ex = abs(e.semi_axes[0]);
    return {e.center - ex, e.center + ex};
}
template <class ScalarT, int DomainD, class TraitsT>
[[nodiscard]] constexpr aabb<DomainD, ScalarT> aabb_of_ellipse2(ellipse<2, ScalarT, DomainD, TraitsT> const& e)
{
...
template <class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<3, ScalarT> aabb_of_ellipse3(ellipse<3, ScalarT, 3, TraitsT> const& e)
{
...
template <class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<1, ScalarT> aabb_of_hemisphere1(hemisphere<1, ScalarT, TraitsT> const& h)
{
    return aabb_of(h.center, h.center + h.normal * h.radius);
}
template <class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<2, ScalarT> aabb_of_hemisphere2(hemisphere<2, ScalarT, TraitsT> const& h)
{
...
template <class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<3, ScalarT> aabb_of_hemisphere3(hemisphere<3, ScalarT, TraitsT> const& h)
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr aabb<D, ScalarT> aabb_of_segment(segment<D, ScalarT> const& s)
{
    return aabb_of(s.pos0, s.pos1);
}

template <int D, class ScalarT>
[[nodiscard]] constexpr aabb<D, ScalarT> aabb_of_triangle(triangle<D, ScalarT> const& t)
{
    return aabb_of(t.pos0, t.pos1, t.pos2);
}

template <int D, class ScalarT>
[[nodiscard]] constexpr aabb<D, ScalarT> aabb_of_quad(quad<D, ScalarT> const& q)
{
    return aabb_of(q.pos00, q.pos10, q.pos11, q.pos01);
}

template <int ObjectD, class ScalarT, int DomainD, class TraitsT>
[[nodiscard]] constexpr aabb<DomainD, ScalarT> aabb_of_box-1(box<ObjectD, ScalarT, DomainD, TraitsT> const& b)
{
...
template <int D, class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<D, ScalarT> aabb_of_capsule(capsule<D, ScalarT, TraitsT> const& c)
{
    return aabb_of(sphere<D, ScalarT>(c.axis.pos0, c.radius), sphere<D, ScalarT>(c.axis.pos1, c.radius));
}

template <int D, class ScalarT, class TraitsT>
[[nodiscard]] constexpr aabb<D, ScalarT> aabb_of_cylinder(cylinder<D, ScalarT, TraitsT> const& c)
{
...
[[nodiscard]] constexpr auto aabb_ofTODO_MANUALLY(PrimA const& pa, PrimB const& pb, PrimsT const&... prims) -> decltype(aabb_of(pa))
{
    auto bb = aabb_of(pa);
...
[[nodiscard]] constexpr auto aabb_ofTODO_MANUALLY(ContainerT const& c, TransformT&& transform = {}) -> decltype(aabb_of(transform(*c.begin())))
{
    auto it = c.begin();
    auto end = c.end();
