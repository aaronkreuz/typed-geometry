...
[[nodiscard]] constexpr vec<D, ScalarT> project_vec(vec<D, ScalarT> const& v, plane<D, ScalarT> const& pl)
{
    return v - pl.normal * dot(v, pl.normal);
}

template <int D, class ScalarT>
[[nodiscard]] constexpr vec<D, ScalarT> project_dir(dir<D, ScalarT> const& v, plane<D, ScalarT> const& pl)
{
    return v - pl.normal * dot(v, pl.normal);
}

template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, plane<D, ScalarT> const& pl)
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, halfspace<D, ScalarT> const& pl)
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_[[maybe_unused]] pos([[maybe_unused]] pos<D, ScalarT> const& p, pos<D, ScalarT> const& q)
{
    return q;
}
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, line<D, ScalarT> const& l)
{
    return l.pos + project(p - l.pos, l.dir);
}

template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, ray<D, ScalarT> const& r)
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, segment<D, ScalarT> const& s)
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, aabb<D, ScalarT> const& b)
{
    return clamp(p, b.min, b.max);
}

template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, aabb<D, ScalarT, boundary_tag> const& b)
{
    auto res = p;
...
[[nodiscard]] constexpr pos<DomainD, ScalarT> project_pos(pos<DomainD, ScalarT> const& p, box<ObjectD, ScalarT, DomainD> const& b)
{
...
template <int ObjectD, class ScalarT, int DomainD>
[[nodiscard]] constexpr pos<DomainD, ScalarT> project_pos(pos<DomainD, ScalarT> const& p, box_boundary<ObjectD, ScalarT, DomainD> const& b)
{
...
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, triangle<3, ScalarT> const& t)
{
    auto n = normal_of(t);
...
template <class ScalarT>
[[nodiscard]] constexpr pos<2, ScalarT> project_pos2(pos<2, ScalarT> const& p, triangle<2, ScalarT> const& t)
{
    if (contains(t, p))
        return p;
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, sphere<D, ScalarT> const& s)
{
    if (contains(s, p))
        return p;
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, sphere_boundary<D, ScalarT> const& sp)
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, sphere<2, ScalarT, 3> const& d)
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, sphere_boundary<2, ScalarT, 3> const& c)
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<2, ScalarT> project_pos2(pos<2, ScalarT> const& p, sphere<1, ScalarT, 2> const& s)
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<2, ScalarT> project_pos2(pos<2, ScalarT> const& p, sphere_boundary<1, ScalarT, 2> const& s)
{
    auto v = perpendicular(s.normal) * s.radius;
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, hemisphere<D, ScalarT> const& h)
{
    auto toP = p - h.center;
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, hemisphere_boundary<D, ScalarT> const& h)
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, hemisphere_boundary_no_caps<D, ScalarT> const& h)
{
    auto dirToP = tg::normalize_safe(p - h.center);
    if (dot(dirToP, h.normal) > ScalarT(0))
...
template <class ScalarT>
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, cylinder<3, ScalarT> const& c) // same as project(tube) for the internal case
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, cylinder_boundary_no_caps<3, ScalarT> const& t)
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, cylinder_boundary<3, ScalarT> const& c) // boundary, including caps
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, inf_cylinder<D, ScalarT> const& c)
{
    if (contains(c, p))
        return p;
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, inf_cylinder_boundary<D, ScalarT> const& c)
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, capsule<3, ScalarT> const& c) // including caps
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, capsule_boundary<3, ScalarT> const& c) // boundary, including caps
{
    auto t = coordinates(c.axis, p);
...
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, cone<3, ScalarT, TraitsT> const& c)
{
...
[[nodiscard]] constexpr pos<3, typename BaseT::scalar_t> project_pos3(pos<3, typename BaseT::scalar_t> const& p, pyramid<BaseT> const& py)
{
    if (contains(py, p))
        return p;
...
template <class BaseT, typename = std::enable_if_t<!std::is_same_v<BaseT, sphere<2, typename BaseT::scalar_t, 3>>>>
[[nodiscard]] constexpr pos<3, typename BaseT::scalar_t> project_pos3(pos<3, typename BaseT::scalar_t> const& p, pyramid_boundary<BaseT> const& py)
{
...
template <class BaseT, typename = std::enable_if_t<!std::is_same_v<BaseT, sphere<2, typename BaseT::scalar_t, 3>>>>
[[nodiscard]] constexpr pos<3, typename BaseT::scalar_t> project_pos3(pos<3, typename BaseT::scalar_t> const& p, pyramid_boundary_no_caps<BaseT> const& py)
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr pos<D, ScalarT> project_pos(pos<D, ScalarT> const& p, inf_cone<D, ScalarT> const& icone)
{
    if (contains(icone, p))
        return p;
...
template <class ScalarT>
[[nodiscard]] constexpr pos<3, ScalarT> project_pos3(pos<3, ScalarT> const& p, inf_cone_boundary<3, ScalarT> const& icone)
{
...
template <class ScalarT>
[[nodiscard]] constexpr pos<2, ScalarT> project_pos2(pos<2, ScalarT> const& p, inf_cone_boundary<2, ScalarT> const& icone)
{
