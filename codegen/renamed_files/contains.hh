...
[[nodiscard]] constexpr auto contains_A const& a, pos(A const& a, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0)) -> decltype(ScalarT(distance(a, p)), false)
{
    return distance(a, p) <= eps;
}

template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_pos(pos<D, ScalarT> const& b, pos<D, ScalarT> const& o, dont_deduce<ScalarT> eps = ScalarT(0))
{
    if (eps > ScalarT(0))
...
[[nodiscard]] constexpr auto contains_A const& a, B const& b, dont_deduce-1(A const& a, B const& b, dont_deduce<typename B::scalar_t> eps = static_cast<typename B::scalar_t>(0))
...
[[nodiscard]] constexpr bool contains_aabb1(aabb<1, ScalarT> const& b, ScalarT const& o, dont_deduce<ScalarT> eps = ScalarT(0))
{
    return b.min.x - eps <= o && o <= b.max.x + eps;
}
template <class ScalarT>
[[nodiscard]] constexpr bool contains_aabb1(aabb<1, ScalarT> const& b, pos<1, ScalarT> const& o, dont_deduce<ScalarT> eps = ScalarT(0))
{
    return b.min.x - eps <= o.x && o.x <= b.max.x + eps;
}
template <class ScalarT>
[[nodiscard]] constexpr bool contains_aabb2(aabb<2, ScalarT> const& b, pos<2, ScalarT> const& o, dont_deduce<ScalarT> eps = ScalarT(0))
{
    return b.min.x - eps <= o.x && o.x <= b.max.x + eps && //
           b.min.y - eps <= o.y && o.y <= b.max.y + eps;
}
template <class ScalarT>
[[nodiscard]] constexpr bool contains_aabb3(aabb<3, ScalarT> const& b, pos<3, ScalarT> const& o, dont_deduce<ScalarT> eps = ScalarT(0))
{
    return b.min.x - eps <= o.x && o.x <= b.max.x + eps && //
           b.min.y - eps <= o.y && o.y <= b.max.y + eps && //
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_aabb-1(aabb<4, ScalarT> const& b, pos<4, ScalarT> const& o, dont_deduce<ScalarT> eps = ScalarT(0))
{
    return b.min.x - eps <= o.x && o.x <= b.max.x + eps && //
           b.min.y - eps <= o.y && o.y <= b.max.y + eps && //
           b.min.z - eps <= o.z && o.z <= b.max.z + eps && //
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_aabb_boundary(aabb_boundary<D, ScalarT> const& b, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_aabb(aabb<D, ScalarT> const& b, aabb<D, ScalarT> const& o, dont_deduce<ScalarT> eps = ScalarT(0))
{
    return contains(b, o.min, eps) && contains(b, o.max, eps);
}

template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_box(box<D, ScalarT> const& b, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto r = p - b.center;
    // TODO: unroll
    for (auto i = 0; i < D; ++i)
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_box_boundary(box_boundary<D, ScalarT> const& b, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto onSomeBoundary = false;
    auto r = p - b.center;
    for (auto i = 0; i < D; ++i)
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_box2(box<2, ScalarT, 3> const& b, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto r = p - b.center;
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_box_boundary2(box_boundary<2, ScalarT, 3> const& b, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto r = p - b.center;
...
[[nodiscard]] constexpr bool contains_ellipse1(ellipse<1, ScalarT, 1, TraitsT> const& e, pos<1, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
[[nodiscard]] constexpr bool contains_ellipse2(ellipse<2, ScalarT, D, TraitsT> const& e, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
// interprets eps as extending the semi_axes by eps, which is very close to the actual distance. It is accurate at the vertices and with less eccentricity.
template <class ScalarT, class TraitsT>
[[nodiscard]] constexpr bool contains_ellipse3(ellipse<3, ScalarT, 3, TraitsT> const& e, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto pc = p - e.center;
    auto a = length(e.semi_axes[0]);
    auto b = length(e.semi_axes[1]);
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_capsule3(capsule<3, ScalarT> const& c, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto r = c.radius + eps;
    return distance_sqr(c.axis, p) <= pow2(r);
}
template <class ScalarT>
[[nodiscard]] constexpr bool contains_capsule_boundary3(capsule_boundary<3, ScalarT> const& c, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_sphere(sphere<D, ScalarT> const& s, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto r = s.radius + eps;
    return distance_sqr(s.center, p) <= pow2(r);
}
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_sphere_boundary(sphere_boundary<D, ScalarT> const& s, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_hemisphere(hemisphere<D, ScalarT> const& s, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_hemisphere_boundary_no_caps(hemisphere_boundary_no_caps<D, ScalarT> const& s, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto c = s.center - eps * s.normal;
    if (dot(s.normal, p - c) < 0)
        return false;
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_hemisphere_boundary(hemisphere_boundary<D, ScalarT> const& s, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    return contains(boundary_no_caps_of(s), p, eps) || contains(caps_of(s), p, eps);
}

template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_halfspace(halfspace<D, ScalarT> const& h, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_triangle2(triangle<2, ScalarT> const& t, pos<2, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    auto pv0 = t.pos0 - p;
    auto pv1 = t.pos1 - p;
    auto pv2 = t.pos2 - p;
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_triangle3(triangle<3, ScalarT> const& t, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_cylinder3(cylinder<3, ScalarT> const& c, pos<3, ScalarT> const& p)
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_inf_cylinder(inf_cylinder<D, ScalarT> const& c, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    return distance_sqr(p, c.axis) <= pow2(c.radius + eps);
}
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_inf_cylinder_boundary(inf_cylinder_boundary<D, ScalarT> const& c, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_cone3(cone<3, ScalarT> const& c, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <int D, class ScalarT>
[[nodiscard]] constexpr bool contains_inf_cone(inf_cone<D, ScalarT> const& c, pos<D, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_frustum3(frustum<3, ScalarT> const& f, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
...
template <class ScalarT>
[[nodiscard]] constexpr bool contains_inf_frustum3(inf_frustum<3, ScalarT> const& f, pos<3, ScalarT> const& p, dont_deduce<ScalarT> eps = ScalarT(0))
{
    for (auto const& pl : f.planes)
        if (signed_distance(p, pl) > eps)
            return false;
